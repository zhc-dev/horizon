...
(
    ...
    `is_active`     TINYINT         NOT NULL DEFAULT 1 COMMENT '账号是否物理启用(1:启用,2:禁用)',
...
(
    ...
    `status`            TINYINT         NOT NULL DEFAULT 0 COMMENT '状态 0:私有(未发布) 1:公开(进行中/已结束) 2:保护(密码访问)',
    `password`          CHAR(60)                 DEFAULT NULL COMMENT '访问密码 (bcrypt, status=2时需要)',
    ...
    UNIQUE KEY `uq_contest_title_active` (`title`, `is_deleted`),
...
(
    ...
    UNIQUE KEY `uq_cq_active` (`contest_id`, `question_id`, `is_deleted`),
    UNIQUE KEY `uq_cq_display_id_active` (`contest_id`, `question_order`, `is_deleted`),
